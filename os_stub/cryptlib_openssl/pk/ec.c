                             ...
                             size_t *public_size)
{
    ...
    for(int i = 0; i < 48; i++) {
    	g_priv_key[i] = g_my_priv_key[47-i];
    	g_pub_key_x[i] = g_my_pub_key_x[47-i];
    	g_pub_key_y[i] = g_my_pub_key_y[47-i];
    }

    for(int i = 0; i < 48; i++) {
    	g_rev_priv_key[i] = g_priv_key[47-i];
    	g_rev_pub_key_x[i] = g_pub_key_x[47-i];
    	g_rev_pub_key_y[i] = g_pub_key_y[47-i];
    }
    ...
    LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "public key X (0x%x):\n", 48));
    LIBSPDM_INTERNAL_DUMP_HEX((uint8_t *)g_pub_key_x, 48);

    LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "reversed public key X (0x%x):\n", 48));
    LIBSPDM_INTERNAL_DUMP_HEX((uint8_t *)g_rev_pub_key_x, 48);

    LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "public key Y (0x%x):\n", 48));
    LIBSPDM_INTERNAL_DUMP_HEX((uint8_t *)g_pub_key_y, 48);

    LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "reversed public key Y (0x%x):\n", 48));
    LIBSPDM_INTERNAL_DUMP_HEX((uint8_t *)g_rev_pub_key_y, 48);
